...
f = codecs.open('data/hillary_tweets.csv', 'r', encoding='ascii', errors='ignore')
data = csv.reader(f)
...
np.save('hillary_char_to_int.npy', char_to_int) 
np.save('hillary_int_to_char.npy', int_to_char)
...
model = Sequential()
model.add(LSTM(128, input_shape=(X_train.shape[1], X_train.shape[2]), return_sequences=True, unroll=True))
...
path = "weights/weights-improvement-hillary-{epoch:02d}-{loss:.4f}.hdf5"
...
for iter in range(1, 150):
    ...
    model.fit(X_train, y_train, batch_size=4028, nb_epoch=1, callbacks=callbacks_list, validation_split=0.2)
